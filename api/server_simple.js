...
function filterHolidays(holidays, filters) {
  ...
  if (filters.data) {
    filtered = filtered.filter((feriado) => {
      // Aceitar tanto DD/MM quanto DD/MM/YYYY
      if (filters.data.includes('/2025')) {
        return feriado.data === filters.data;
      } else {
        // Se só DD/MM, comparar apenas dia e mês
        const [diaFiltro, mesFiltro] = filters.data.split("/");
        const [diaFeriado, mesFeriado] = feriado.data.split("/");
        return diaFiltro === diaFeriado && mesFiltro === mesFeriado;
      }
    });
  }
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      '/feriados/buscar': 'GET - Busca feriados por data específica (parâmetro ?data=DD/MM)',
...
// Buscar feriados por data específica usando query parameter
app.get('/feriados/buscar', (req, res) => {
  try {
    const data = req.query.data;
    
    if (!data) {
      return res.status(400).json({ 
        error: 'Parâmetro data é obrigatório. Use ?data=DD/MM (ex: ?data=25/12)' 
      });
    }

    // Validar formato da data (DD/MM ou DD/MM/YYYY)
    const dateRegex = /^(\d{1,2})\/(\d{1,2})(\/\d{4})?$/;
    if (!dateRegex.test(data)) {
      return res.status(400).json({ 
        error: 'Formato de data inválido. Use DD/MM ou DD/MM/YYYY (ex: 25/12 ou 25/12/2025)' 
      });
    }
